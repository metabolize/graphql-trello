...
const Query = {
  getBoard: async (_, { boardId }, { trello }) =>
    Board.create(await trello.get(`/1/boards/${boardId}`)),

  getList: async (_, { listId }, { trello }) =>
    List.create(await trello.get(`/1/lists/${listId}`)),

  getCard: async (_, { cardId }, { trello }) =>
    Card.create(
      await trello.get(`/1/cards/${cardId}`, { customFieldItems: 'true' })
    ),

  getMember: async (_, { memberId }, { trello }) =>
    Member.create(await trello.get(`/1/members/${memberId}`)),
}
...
class List {
  ...
  async cards(_, { trello }) {
    const { id } = this
    return trello
      .get(`/1/lists/${id}/cards`, { customFieldItems: 'true' })
      .map(Card.create)
  }
}
...
class Card {
  ...
  async comments(_, { trello }) {
    const { id } = this
    return (await trello.get(`/1/cards/${id}/actions`, {
      filter: 'commentCard',
    })).map(Comment.create)
  }
}
...
class Member {
  ...
  async cards(_, { trello }) {
    const { id } = this
    return (await trello.get(`/1/members/${id}/cards`, {
...
class CustomFieldItem {
  constructor({ id, idCustomField, idModel, value }) {
    ...
    this.boolValue =
      value.checked === undefined ? undefined : Boolean(value.checked)
  }
