...
class List {
  ...
  async cards(_, {trello}) {
    return trello.get(`/1/lists/${this.id}/cards`).map(Card.create)
  }
}

class Card {
  constructor({id, name, url, dateLastActivity, labels}) {
    this.id = id;
    this.name = name;
    this.url = url;
  ...
  async members(_, {trello}) {
    const {id} = this
    return (await trello.get(`/1/cards/${this.id}/members`)).map(Member.create)
  }

  async comments(_, {trello}) {
    const {id} = this
    return (await trello.get(`/1/cards/${this.id}/actions`, { filter: "commentCard" })).map(Comment.create)
  }
}
...
class Member {
  ...
  async cards(_, {trello}) {
    const{id} = this
    return (await trello.get(`/1/members/${this.id}/cards`)).map(Card.create)
  }
}
